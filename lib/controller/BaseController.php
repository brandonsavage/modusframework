...
abstract class BaseController implements ControllerI
{
	protected $request;
	protected $response;
	protected $module;
	...
	public function authenticate()
	{
		...
		{
			case 401:
				$e = new ActionException('Unauthenticated access attempt.');
				$e->setUserMessage('This action requires authentication. You must log in.');
				throw $e;
				break;
				
			case 403:
				$e = new ActionException('The role this user has does not permit this action to be performed.');
	...
	public function initialize()
	{
		...
		if(!$module && !$action)
		{
			throw new ModusException('cannot find module and action!', 404);
		}
	...
	public function execute()
	{
		...
		{
			$ex = new ModusException('There was a database error:' . $e);
	...
	{
		if($this->formatter instanceof FormatterI)
		{
	...
	public function setResponse(ResponseI $response)
	{
	...
	{
		if($this->response instanceof ResponseI)
		{
			return $response;
		}
		return null; // to be explicit.
